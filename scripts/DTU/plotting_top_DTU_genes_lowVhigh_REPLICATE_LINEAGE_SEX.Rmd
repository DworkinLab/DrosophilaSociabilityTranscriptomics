---
title: "plotting DTU genes with replicate lineage and sex"
output:
...
## if using emmean set y = emmean ## but if changing to type = "response" then set y = response
  ...
  plotting_df$selection_new <- paste0(plotting_df$transcript, "_", plotting_df$selection)
  ...
  gene_countMatrix$selection_new <- paste0(gene_countMatrix$transcript, "_", gene_countMatrix$selection)
  
  
  levels_out <- c()
  for (i in 1:length(unique(plotting_df$transcript))) {
    
    low_out <- paste0(as.character(unique(plotting_df$transcript)[i]), "_Low")
    control_out <- paste0(as.character(unique(plotting_df$transcript)[i]), "_Control")
    high_out <- paste0(as.character(unique(plotting_df$transcript)[i]), "_High")
    
    levels_out <- c(levels_out, low_out)
    levels_out <- c(levels_out, control_out)
    levels_out <- c(levels_out, high_out)
  }
  
  
  gene_countMatrix$selection_new <- factor(gene_countMatrix$selection_new, levels = levels_out)
  ...
  if (toLog == TRUE) {
    ...
    gene_countMatrix$countsPlus <- log2(gene_countMatrix$countsPlus)
        ...
        ggplot() + 
    geom_point(data = plotting_df, aes(x = selection_new, y = response),
               position = pd) +
    
    geom_errorbar(data = plotting_df, aes(x = selection_new, y = response,
                                        ymin = lower.CL, ymax = upper.CL),
                                          width = 0.7, position = pd, alpha = 0.65)
    ...
    geom_errorbar(data = plotting_df, aes(x = selection_new, y = response,
                                        ymin = lower.CL, ymax = upper.CL),
                                          width = 0.7, alpha = 0.65) +
     geom_jitter(data = gene_countMatrix, aes(x = selection_new, y = countsPlus, color = lineage, shape = sex),size = 1.3, alpha = 0.5, width = 0.32) + 
      scale_color_manual(values = unname(pals::okabe(n = 4))) +
      #scale_color_manual(values = c("1" = "dodgerblue", "2" = "black", "3" = "orangered", "4" = )) +
    ggtitle(geneName) +
    xlab("Transcript") +
    #xlab("") +
    #ylab("") +
    ...
    theme( plot.title = element_text(face = "italic"),axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
  }
  else {
    ggplot() + 
